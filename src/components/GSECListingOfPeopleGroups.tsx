...
import { gsecListingOfPeopleGroupsQuery } from '../common/query';


const remoteData = (query: { page: number; search: string; }) => {
    console.log(query.search)
    return client.query({
        query: gsecListingOfPeopleGroupsQuery,
        variables: {
        ...
        console.log(res)
        return {
            data: res.data.gsec_listing_of_people_groups,
            page: query.page,
            totalCount: res.data.gsec_listing_of_people_groups_aggregate.aggregate.count
        }
    })
}

export function GSECListingOfPeopleGroups() {
    ...
    return (
        <IonContent>
            <StyledWrap>
                <StyledH3>GSEC Listing Of People Groups</StyledH3>
                <div style={{ maxWidth: "100%" }}>
                    <Table title="GSEC Listing Of People Groups" columns={columns} remoteData={remoteData} ></Table>
                </div>
            </StyledWrap>
        </IonContent>
    );
