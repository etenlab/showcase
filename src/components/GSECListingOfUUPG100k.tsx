...
import { gsecListingOfUupg100kQuery } from '../common/query';

const remoteData = (query: { page: number; search: string; }) => {
    console.log(query.search)
    return client.query({
        query: gsecListingOfUupg100kQuery,
        variables: {
        ...
        console.log(res)
        return {
            data: res.data.gsec_listing_of_uupg_100k,
            page: query.page,
            totalCount: res.data.gsec_listing_of_uupg_100k_aggregate.aggregate.count
        }
    })
}

export function GSECListingOfUUPG100k() {
    ...
    return (
        <IonContent>
            <StyledWrap>
                <StyledH3>GSEC Listing Of UUPG 100k</StyledH3>
                <div style={{ maxWidth: "100%" }}>
                    <Table title="GSEC Listing Of UUPG 100k" columns={columns} remoteData={remoteData} ></Table>
                </div>
            </StyledWrap>
        </IonContent>
    );
