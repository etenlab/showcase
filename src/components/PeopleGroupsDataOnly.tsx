...
import { peopleGroupsDataOnlyQuery } from '../common/query';

const remoteData = (query: { page: number; search: string; }) => {
    console.log(query.search)
    return client.query({
        query: peopleGroupsDataOnlyQuery,
        variables: {
        ...
        console.log(res)
        return {
            data: res.data.people_groups_data_only,
            page: query.page,
            totalCount: res.data.people_groups_data_only_aggregate.aggregate.count
        }
    })
}

export function PeopleGroupsDataOnly() {
    ...
    return (
        <IonContent>
            <StyledWrap>
                <StyledH3>People Groups Data Only</StyledH3>
                <div style={{ maxWidth: "100%" }}>
                    <Table title="People Groups Data Only" columns={columns} remoteData={remoteData} ></Table>
                </div>
            </StyledWrap>
        </IonContent>
    );
