    ...
    if (data) {
      if (TablesMeta[tName].detailsPanel) {
        ...
        TablesMeta[tName].detailsPanel!.tableNames!.forEach((tbl) => {
          fieldsA[tbl] = TablesMeta[tbl.replaceAll('_', '-')].fields.map(
            (value) => value.field
          );
        });
        // console.log(fieldsA);
        ...
        client
          ...
          .then((res) => {
            ...
            for (
              var i = 0;
              i < TablesMeta[tName].detailsPanel!.tableNames!.length;
              i++
            ) {
              if (
                res.data[TablesMeta[tName].detailsPanel!.tableNames![i]]
                  .length > 0
              ) {
                let tableData =
                  res.data[TablesMeta[tName].detailsPanel!.tableNames![i]][0];
                subDataTables.push(
                  <div css={styles.detailsPanelBox} key={i}>
                    <h4>
                      {
                        TablesMeta[
                          TablesMeta[tName].detailsPanel!.tableNames![
                            i
                          ].replaceAll('_', '-')
                        ].title
                      }
            ...
            html = (
              <StyledTable>
                <div
                  ...
                  css={styles.detailsPanelWrapper}
                >
                  {subDataTables}
                </div>
              </StyledTable>
    ...
    <IonContent>
      <StyledWrap>
        ...
        <div style={{ maxWidth: '100%' }}>
          <Table
            title={TablesMeta[tName].title}
            columns={TablesMeta[tName].fields}
            remoteData={remoteData}
            detailsPanel={TablesMeta[tName].detailsPanel ? true : false}
            remoteSubData={remoteSubData}
          ></Table>
        </div>
      </StyledWrap>
    </IonContent>
